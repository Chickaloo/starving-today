...
// CommentEdit implements the PUT /comments/{commentid} endpoint to edit a comment
func CommentEdit(w http.ResponseWriter, r *http.Request) {
	...
	query := fmt.Sprintf("UPDATE comment\nSET comment=\"%s\"\nWHERE comment_id=\"%s\"", rdata.Comment, params["commentid"])
	result, err := db.Connection.Exec(query)
	if err != nil {
		if *Debug {
			fmt.Println("Comment Edit Failed: ", err.Error())
		}
		res.Content = fmt.Sprintf("Comment Edit Failed: %s", err.Error())
		Respond(w, result, http.StatusInternalServerError)
		return
	}
	Respond(w, rdata, http.StatusOK)
...
// CommentsGetByUserID implements the GET /comments/user/{userid} endpoint to get a user's comments
func CommentsGetByUserID(w http.ResponseWriter, r *http.Request) {
	var res Response
	var rdata Comments
	params := mux.Vars(r)

	rows, err := db.Connection.Query(fmt.Sprintf("SELECT comment_id, date, comment, recipe_id, user_id, poster_id FROM comment WHERE user_id=\"%s\"", params["userid"]))
	if err != nil {
		fmt.Println("Query Error: " + err.Error())
		Respond(w, res, http.StatusInternalServerError)
		return
